### ABOUT ME




I'm a proven leader with experience in developing high-performance solutions, driving results and lean problem-solving.
I've been able to leverage my work via concrete communication,effective collaboration and appropiate domain specefic skills in many situations.

I helped launch the very first predictive analytics solution in the history of DMRC,where I've been able to implement and improve processes that will leverage across the entire network of Rails.

Colleagues know me as an innovative thinker and productive worker that brings new ideas to most discussions and meetings. I tend to foster an ongoing dialogue between groups that I am a part of, to ensure every person can add to the pool of meaning and that, as a group, we make the best decisions possible.

For the future, I'm excited to build exponentially on skills like these.

In my free time I love to read , play videogames or contribute to opensource.


  ## Top Skills :

<div style={{ padding: '10px 30px', backgroundColor: 'yellow' }}>
  <h2>Python</h2>
</div>
<div style={{ padding: '10px 30px', backgroundColor: 'green' }}>
  <h2>Django</h2>
</div>
<div style={{ padding: '10px 30px', backgroundColor: 'lightblue' }}>
  <h2>Docker</h2>
</div>
<div style={{ padding: '10px 30px', backgroundColor: 'tomato' }}>
  <h2>Javascript</h2>
</div>
<div style={{ padding: '10px 30px', backgroundColor: 'orange' }}>
  <h2>Data Analytics</h2>
</div>


